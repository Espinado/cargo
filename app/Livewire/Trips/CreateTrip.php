...
class CreateTrip extends Component
{
    ...
    /** third-party carriers (for autocomplete / name suggestions when creating new) */
    ...
    /** When creating trip from order(s): one ID (from_order) or several (from_orders). After save all get trip_id + CONVERTED. */
    ...
    public function mount(): void
    {
        ...
        // external carriers from directory (for select)
        ...
        // third party carriers list for name suggestions when creating new (datalist)
        ...
        // Prefill from order(s): from_order=ID or from_orders=1,2,3. Merge steps/cargos from all; link all to trip on save.
        $fromOrderId = request()->query('from_order');
        $fromOrdersParam = request()->query('from_orders');
        $orderIds = [];
        if ($fromOrdersParam && is_string($fromOrdersParam)) {
            $orderIds = array_filter(array_map('intval', explode(',', $fromOrdersParam)));
        }
        if ($fromOrderId && is_numeric($fromOrderId)) {
            $id = (int) $fromOrderId;
            if (!in_array($id, $orderIds, true)) {
                array_unshift($orderIds, $id);
            }
        }
        if (!empty($orderIds)) {
            $this->from_order_ids = array_values(array_unique($orderIds));
            $this->from_order_id = $this->from_order_ids[0];
            $this->selected_order_ids_input = $this->from_order_ids;
            TransportOrder::whereIn('id', $this->from_order_ids)
                ->whereIn('status', ['draft', 'quoted'])
                ->update(['status' => OrderStatus::CONFIRMED->value]);
    ...
    /**
     * Prefill trip form from one or more orders: merge steps and cargos, set expeditor/dates from first.
     */
    protected function prefillFromOrderIds(array $orderIds): void
    {
        if (empty($orderIds)) {
            return;
        }
        $orders = TransportOrder::with(['expeditor', 'steps', 'cargos'])
            ->whereIn('id', $orderIds)
            ->orderByRaw('FIELD(id, ' . implode(',', array_map('intval', $orderIds)) . ')')
            ->get();
        if ($orders->isEmpty()) {
            return;
        }
        $first = $orders->first();
        $this->expeditor_id = $first->expeditor_id;
        $this->hydrateExpeditor();
        $dateFrom = $first->requested_date_from?->format('Y-m-d');
        $dateTo = $first->requested_date_to?->format('Y-m-d');
        foreach ($orders as $o) {
            if ($o->requested_date_from && (!$dateFrom || $o->requested_date_from->format('Y-m-d') < $dateFrom)) {
                $dateFrom = $o->requested_date_from->format('Y-m-d');
            }
            if ($o->requested_date_to && (!$dateTo || $o->requested_date_to->format('Y-m-d') > $dateTo)) {
                $dateTo = $o->requested_date_to->format('Y-m-d');
            }
            if ($o->cargos->isNotEmpty()) {
                $fromCargos = $o->cargos->pluck('requested_date_from')->filter()->map(fn ($d) => $d->format('Y-m-d'));
                $toCargos = $o->cargos->pluck('requested_date_to')->filter()->map(fn ($d) => $d->format('Y-m-d'));
                if ($fromCargos->isNotEmpty() && (!$dateFrom || $fromCargos->min() < $dateFrom)) {
                    $dateFrom = $fromCargos->min();
                }
                if ($toCargos->isNotEmpty() && (!$dateTo || $toCargos->max() > $dateTo)) {
                    $dateTo = $toCargos->max();
                }
            }
        }
        ...
        $this->currency = $first->currency ?? 'EUR';
        $this->customs = (bool) $first->customs;
        $this->customs_address = $first->customs_address;
        ...
        foreach ($orders as $orderIndex => $order) {
            ...
            foreach ($order->steps->sortBy('order') as $os) {
                $cities = $os->country_id && function_exists('getCitiesByCountryId')
                    ...
                    : [];
                $cityId = $os->city_id;
                if (!$cityId && $os->address && !empty($cities)) {
                ...
                $this->cargos[] = [
                    ...
                    'items'                    => [
                        [
                            ...
                            'pallets'         => $oc->pallets,
                            'units'           => $oc->units,
                            'net_weight'      => $oc->net_weight ?? $oc->weight_kg,
                            'gross_weight'    => $oc->gross_weight ?? $oc->weight_kg,
                            'tonnes'          => $oc->tonnes,
                            'volume'          => $oc->volume_m3,
    ...
    /**
     * Resolve city_id from address when order step has no city_id.
     * E.g. address "Vilnius, noliktava" → find city "Vilnius" in country and return its id.
     */
    protected function resolveCityIdFromAddress(string $address, array $cities): mixed
    {
        $address = trim($address);
        if ($address === '') {
            return null;
        }
        $firstPart = trim(explode(',', $address)[0]);
        if ($firstPart === '') {
            return null;
        }
        $firstLower = mb_strtolower($firstPart);
        foreach ($cities as $id => $city) {
            $name = $city['name'] ?? '';
            if ($name !== '' && mb_strtolower(trim($name)) === $firstLower) {
                return $id;
            }
        }
        return null;
     ...
     * - '123' => carrier_company_id = 123 (our or external from directory)
     */
    ...
    public function save(): void
    {
        ...
        try {
            ...
            // third party flow: create company only when "new"; always ensure truck + trailer for external carrier
            if ($isThirdPartyFlow) {
                if ($isNewThirdParty) {
            ...
            $orderIdsToLink = !empty($this->from_order_ids) ? $this->from_order_ids : ($this->from_order_id ? [$this->from_order_id] : []);
            if (!empty($orderIdsToLink)) {
                TransportOrder::whereIn('id', $orderIdsToLink)->update([
                    'trip_id' => $trip->id,
