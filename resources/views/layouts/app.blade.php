...
<body class="bg-gray-100 h-screen flex overflow-hidden relative">
    ...
    {{-- ===== Main content (как во Fleet: min-h-0 для корректного скролла в main) ===== --}}
    <div class="flex-1 flex flex-col min-h-0 w-0 min-w-0">
        ...
        {{-- ===== Page Content ===== --}}
        <main id="main-content" class="flex-1 overflow-y-auto overflow-x-hidden p-6 min-h-0">
            {{ $slot ?? '' }}
            @yield('content')
        </main>
    </div>
                        ...
                        $el.off('change.lwSelect2').on('change.lwSelect2', function (e) {
                            ...
                            if (wrapper && !wrapper.classList.contains('select2-spinner-wrapper')) {
                                ...
                                if (!spinner) {
                                    ...
                                    spinner.className = 'select2-inline-spinner absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 border-2 border-gray-200 border-t-amber-500 rounded-full animate-spin pointer-events-none';
    ...
    {{-- Общая карта (страница /map): Leaflet или Google Maps по MAP_PROVIDER --}}
    <script>
    (function() {
