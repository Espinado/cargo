<div class="p-4 sm:p-6 w-full max-w-[1600px] mx-auto">
    ...
    <div class="bg-white shadow rounded-xl border border-gray-200 p-4 sm:p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-2">{{ __('app.orders.edit.title') }}</h2>
        <p class="text-sm text-gray-500 mb-4">{{ $transportOrder->number }}</p>
        ...
        <form wire:submit.prevent="save" class="space-y-4">
            ...
            @if($status === 'converted')
                <div class="rounded-lg border border-blue-200 bg-blue-50/50 p-4">
                    <h3 class="text-sm font-semibold text-gray-800 mb-1">{{ __('app.orders.trip_link_label') }}</h3>
                    <p class="text-xs text-gray-600 mb-2">{{ __('app.orders.trip_link_hint') }}</p>
                    <select wire:model="trip_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                        <option value="">— {{ __('app.orders.trip_link_empty') }} —</option>
                        @foreach($trips as $t)
                            <option value="{{ $t->id }}">{{ __('app.orders.trip_link_option', ['id' => $t->id, 'from' => $t->start_date?->format('d.m.Y'), 'to' => $t->end_date?->format('d.m.Y')]) }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
            ...
            <div>
                ...
                <textarea wire:model="notes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></textarea>
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                @endforeach
                <button type="button" wire:click="addStep" class="text-sm text-blue-600 hover:underline">+ {{ __('app.orders.add_step') }}</button>
            </div>
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                @endforeach
                <button type="button" wire:click="addCargo" class="text-sm text-blue-600 hover:underline">+ {{ __('app.orders.add_cargo') }}</button>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">{{ __('app.orders.edit') }}</button>
                <a href="{{ route('orders.show', $transportOrder) }}" wire:navigate class="text-gray-600 hover:text-gray-900 text-sm">{{ __('app.orders.show.back') }}</a>
            </div>
        </form>
    </div>
</div>
