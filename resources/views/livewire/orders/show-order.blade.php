...
<div class="p-4 sm:p-6 max-w-5xl mx-auto">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @else
                <a href="{{ route('trips.show', $transportOrder->trip_id) }}" wire:navigate
                   class="inline-flex items-center gap-1.5 bg-blue-600 text-white text-sm font-medium px-3 py-2.5 rounded-lg shadow hover:bg-blue-700 min-h-[44px] touch-manipulation">
    ...
    <div class="bg-white shadow rounded-xl border border-gray-200 overflow-hidden">
        ...
        @if($transportOrder->steps->isNotEmpty())
            <div class="border-t border-gray-200 p-4 sm:p-6">
                ...
                {{-- Расчёт километража по маршруту (Google/HERE/ORS), тип ТС: грузовик — помощник для тарифов --}}
                <div class="mt-4 pt-4 border-t border-gray-100">
                    ...
                    <div class="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2">
                        ...
                        @if($routeSummaryError)
                            <span class="block w-full text-sm text-amber-700 break-words">{{ $routeSummaryError }}</span>
                            @if($routeCalcConfigHint ?? false)
                                <span class="text-xs text-gray-500 block w-full">
    ...
    {{-- Модалка: добавить заказ в существующий рейс (только незавершённые); один заказ — один рейс --}}
