<div class="p-4 sm:p-6 max-w-5xl mx-auto space-y-6 bg-gray-50 min-h-screen">
    ...
    <div x-show="fileUploading"
        ...
        {{-- Virsraksts --}}
        <header>
            <h2 class="text-2xl sm:text-3xl font-bold mb-1 text-gray-900">✏️ {{ __('app.truck.edit.title') }}</h2>
            <p class="text-gray-600 text-sm sm:text-base">{{ __('app.truck.edit.subtitle') }}</p>
        </header>
...
<section class="space-y-4">
    ...
    <div class="text-sm text-gray-600 bg-gray-50 border rounded-xl p-3">
        <div class="flex items-start gap-2">
            ...
            <div>Neobligāti lauki. Ja norādīti datumi, <span class="font-medium">{{ __('app.truck.show.expires') }}</span> ir jābūt pēc <span class="font-medium">{{ __('app.truck.show.issued') }}</span>.</div>
        </div>
    </div>
</section>
    ...
    <div>
        ...
        {{-- ========================================================= --}}
        {{-- 5️⃣ DARBĪBAS — FIKSĒTA APAKŠĒJĀ JOSLA --}}
        {{-- ========================================================= --}}
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg p-4
            ...
            <button type="submit"
                ...
                <span wire:loading.remove wire:target="save">💾 {{ __('app.truck.edit.save') }}</span>

                <span wire:loading wire:target="save" class="inline-flex items-center gap-2">
                    <div class="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></div>
                    {{ __('app.truck.edit.saving') }}
                </span>
