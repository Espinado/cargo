...
@else
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end mb-4">
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('app.stats.date_from') }}</label>
                <input type="date" wire:model.live="mileagePeriodFrom"
                       class="w-full px-3 py-2 rounded-xl border border-gray-300 text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('app.stats.date_to') }}</label>
            ...
            <table class="min-w-full text-sm border border-gray-200 rounded-xl overflow-hidden">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">{{ __('app.truck.show.mileage_col_date') }}</th>
                        <th class="px-4 py-2 text-right font-semibold text-gray-700">{{ __('app.truck.show.mileage_col_km') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($mileageStats['trips'] as $t)
                    <tr class="border-t border-gray-100">
                        <td class="px-4 py-2">
                            <a href="{{ route('trips.show', $t['id']) }}" wire:navigate class="text-blue-600 hover:underline font-medium">#{{ $t['id'] }}</a>
                        </td>
                        <td class="px-4 py-2 text-gray-700">{{ $t['start_date'] ? \Carbon\Carbon::parse($t['start_date'])->format('d.m.Y') : '—' }}</td>
    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-3">
            <div class="border rounded-lg p-2 flex items-center justify-center h-48 bg-gray-50">
                @if($truck->tech_passport_photo_url)
                    <a href="{{ $truck->tech_passport_photo_url }}" target="_blank" rel="noopener">
                        <img src="{{ $truck->tech_passport_photo_url }}" alt="Tech Passport"
                             class="h-full object-contain rounded" loading="lazy">
                    </a>
                @else
