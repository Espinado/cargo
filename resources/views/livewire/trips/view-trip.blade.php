...
<div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-8">
    ...
    {{-- ===================================================================== --}}
    {{-- 📱 MOBILE: расчёт маршрута (км, время, оптимальный) — на десктопе в блоке ниже --}}
    {{-- ===================================================================== --}}
    <div class="md:hidden bg-white dark:bg-gray-900 shadow rounded-xl p-4 border border-gray-100 dark:border-gray-800 space-y-3">
        <div>
            <div class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('app.trip.show.route_calc_title') }}</div>
        ...
        <div class="flex flex-col gap-2">
            <div class="flex flex-col sm:flex-row gap-2">
                <button type="button" wire:click="calculateRouteDistance"
                        ...
                        class="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 min-h-[44px] touch-manipulation disabled:opacity-50 transition-colors w-full sm:min-w-[10rem]">
                ...
                <button type="button" wire:click="calculateOptimalRoute"
                        wire:loading.attr="disabled"
                        class="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg bg-violet-600 text-white text-xs font-semibold hover:bg-violet-700 focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 min-h-[44px] touch-manipulation disabled:opacity-50 transition-colors w-full sm:min-w-[10rem]">
            ...
            @if($routeSummary ?? null)
                <div class="flex flex-col gap-2 min-w-0">
                    <span class="inline-flex flex-col sm:flex-row sm:items-baseline gap-0.5 rounded-lg bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 text-blue-800 dark:text-blue-200 text-sm">
                        <span class="text-blue-600 dark:text-blue-400 font-medium shrink-0">{{ __('app.orders.route_calc.distance') }}:</span>
                        <strong>{{ number_format($routeSummary['distance_km'], 0, '.', ' ') }} km</strong>
                    </span>
                    <span class="inline-flex flex-col sm:flex-row sm:items-baseline gap-0.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 px-3 py-1.5 text-emerald-800 dark:text-emerald-200 text-sm">
                        <span class="text-emerald-600 dark:text-emerald-400 font-medium shrink-0">{{ __('app.orders.route_calc.duration') }}:</span>
        ...
        @if($routeSummaryOptimal !== null)
            <div class="pt-3 mt-3 border-t border-gray-200 dark:border-gray-600 space-y-3">
                ...
                <div class="flex flex-wrap items-baseline gap-x-4 gap-y-2 text-sm">
                    <span class="inline-flex flex-col sm:flex-row sm:items-center gap-0.5 rounded-lg bg-violet-50 dark:bg-violet-900/30 px-3 py-2 text-violet-800 dark:text-violet-200 min-w-0">
                        <strong>{{ number_format($routeSummaryOptimal['distance_km'], 0, '.', ' ') }} km</strong>
                        <span class="text-violet-600 dark:text-violet-400 text-xs">{{ __('app.orders.route_calc.duration') }}: {{ $this->formatRouteDuration($routeSummaryOptimal['duration_minutes']) }}</span>
                    </span>
                ...
                @if(!empty($routeSuggestedOrderLabels))
                    <div>
                        ...
                        <div class="text-sm text-gray-700 dark:text-gray-300 break-words leading-relaxed space-y-0.5">
